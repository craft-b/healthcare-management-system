    ...
    <div class="page-container">
      <div class="page-header">
        <div>
          ...
          <p class="page-subtitle">{{ invoices.length }} invoice(s) · {{ pendingCount }} pending</p>
        </div>
        ...
        <div class="card mb-4">
          ...
          <div class="card-body">
            <form [formGroup]="invoiceForm" (ngSubmit)="createInvoice()">
              ...
              @if (formError) {
                <div class="alert alert-danger mt-2"><i class="bi bi-exclamation-circle-fill"></i>{{ formError }}</div>
              }
      ...
      @if (loading) {
        ...
        <div class="card">
          <div class="empty-state"><i class="bi bi-receipt"></i><p>No invoices found.</p></div>
        </div>
      } @else {
        <div class="card">
                  ...
                  <tr>
                    ...
                    <td>
                      <div style="display:flex;gap:.4rem;flex-wrap:wrap;align-items:center">
                        ...
                        <!-- Patient: can pay or submit claim on their own invoices -->
                        @if (auth.role === 'PATIENT' && inv.status === 'PENDING') {
                          <button class="btn btn-success btn-sm" (click)="pay(inv)">
                            <i class="bi bi-credit-card"></i>Pay
                          </button>
                          <button class="btn btn-outline btn-sm" (click)="submitClaim(inv)">
                            <i class="bi bi-file-earmark-text"></i>Claim
                          </button>
                        }

                        <!-- Admin: full override — can mark any invoice paid or cancel it -->
                        @if (auth.role === 'ADMIN' && inv.status === 'PENDING') {
                          <button class="btn btn-success btn-sm" (click)="pay(inv)">
                        ...
                        <!-- Provider: read-only — they issued the invoice, patient settles it -->
                        @if (auth.role === 'PROVIDER' && inv.status === 'PENDING') {
                          <span style="font-size:.78rem;color:var(--muted);font-style:italic">Awaiting patient</span>
                        }

                        @if (inv.status === 'PAID') {
                          <span style="font-size:.8rem;color:var(--success);font-weight:600">
                            <i class="bi bi-check-circle-fill"></i> Paid {{ inv.paidAt | date:'MMM d' }}
                          </span>
                        }

                        @if (inv.status === 'CLAIM_SUBMITTED') {
                          <span style="font-size:.78rem;color:var(--info);font-weight:600">
                            <i class="bi bi-clock-history"></i> Claim pending
                          </span>
                        }

                        @if (inv.status === 'CLAIM_APPROVED') {
                          <span style="font-size:.78rem;color:var(--success);font-weight:600">
                            <i class="bi bi-shield-check"></i> Claim approved
                          </span>
                        }

                        @if (inv.status === 'CLAIM_DENIED') {
                          <span style="font-size:.78rem;color:var(--danger);font-weight:600">
                            <i class="bi bi-shield-x"></i> Claim denied
                          </span>
                        }
...
export class BillingComponent implements OnInit {
  ...
  pay(inv: InvoiceDto) {
    const label = this.auth.role === 'ADMIN' ? 'Mark this invoice as paid?' : `Pay ${inv.amount} for "${inv.serviceDescription}"?`;
    if (!confirm(label)) return;
    this.invoiceService.pay(inv.id!).subscribe(updated => {
      const i = this.invoices.findIndex(x => x.id === inv.id);
      if (i !== -1) this.invoices[i] = updated;
    });
  }
  ...
  statusBadge(status: string): string {
    const map: Record<string, string> = {
      ...
      CLAIM_SUBMITTED:  'badge-info',
      CLAIM_APPROVED:   'badge-success',
      CLAIM_DENIED:     'badge-danger'
    };
